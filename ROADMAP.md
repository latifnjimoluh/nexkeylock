...
- [x] **Accès d'urgence** (crate `nex-urgence`) : matériel d'accès **scellé** vers un contact via le partage hybride PQ + **mécanisme à délai** (demande → refus possible → libération à l'échéance ; le serveur ne déchiffre rien). 6 tests (+ sérialisation).
- [x] **Commandes CLI** `share` / `emergency` / `passkey` câblées dans `nex-console` (identité de partage et passkeys stockées chiffrées dans le coffre) ; 3 tests e2e (partage entre deux coffres, accès d'urgence avec délai, passkey création/assertion avec compteur persisté).
